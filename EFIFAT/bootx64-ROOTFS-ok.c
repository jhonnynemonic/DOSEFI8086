...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    Status = uefi_call_wrapper(Root->Read, 3, Root, &BufferSize, NULL);
    if (Status != EFI_BUFFER_TOO_SMALL) {
        Print(L"[ERR] No se pudo obtener tamaño de FileInfo\n");
        return Status;
    }
    ...
    Print(L"\n=== CONTENIDO DE FS0:\\EFI ===\n");

    EFI_FILE_INFO *Info;
    UINTN InfoSize = 0;

    // 1. Obtener tamaño necesario para EFI_FILE_INFO
    Status = uefi_call_wrapper(Dir->Read, 3, Dir, &InfoSize, NULL);
    if (Status != EFI_BUFFER_TOO_SMALL) {
    Print(L"Error obteniendo tamaño FileInfo\n");
    return Status;
    }
    ...
    // 2. Leer entradas del directorio en bucle
    while (TRUE) {
    UINTN Size = InfoSize;
