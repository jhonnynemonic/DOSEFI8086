...
void int13h(void)
{
    ...
    uint8_t dl = get_dx() & 0xFF;   // DL = drive number

    // Solo soportamos HDD 0x80
    if (dl != 0x80) {
        set_ah(1);
        cpu.eflags |= FLAG_CF;
        return;
    ...
    // Geometría HDD FAT16 típica
    const uint16_t SPT   = 63;   // sectores por pista
    const uint16_t HEADS = 16;   // cabezas
    const uint16_t CYLS  = 1024; // cilindros (máx CHS BIOS)
    ...
    switch (ah)
    {
        ...
        case 0x02:
        {
            ...
            uint16_t cylinder = (cx >> 8) | ((cx & 0xC0) << 2);  // CH + CL bits 6-7
        ...
        // ---------------------------------------------------------
        // AH = 08h — GET DRIVE PARAMETERS (HDD)
        // ---------------------------------------------------------
        case 0x08:
        {
            set_ah(0);
            ...
            // CH = cilindros - 1
            // CL = sectores por pista (bits 0-5) + cilindro bits 8-9 (bits 6-7)
            uint16_t ch = (CYLS - 1) & 0xFF;
            uint16_t cl = (SPT & 0x3F) | (((CYLS - 1) >> 8) << 6);

            set_cx((ch << 8) | cl);

            // DH = cabezas - 1, DL = drive number (0x80)
            set_dx(((HEADS - 1) << 8) | 0x80);

            set_bx(0); // no DMA info
            cpu.eflags &= ~FLAG_CF;
            return;
        }

        // ---------------------------------------------------------
        // AH = 15h — GET DISK TYPE (HDD)
        // ---------------------------------------------------------
        case 0x15:
        {
            set_ax(3);      // 03h = fixed disk present
            set_bx(0);
            set_cx(0);
            cpu.eflags &= ~FLAG_CF;
            return;
