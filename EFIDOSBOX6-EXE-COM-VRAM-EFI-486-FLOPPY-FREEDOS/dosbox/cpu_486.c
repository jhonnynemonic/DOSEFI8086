...
static uint32_t ea16(ModRM m)
{
    ...
    case 0: // no displacement
        switch (m.rm) {
            ...
            case 6: {
                uint16_t addr = mem_read16((cpu.cs<<4) + get_ip());
                cpu.eip += 2;
                return addr;
            }
    ...
    case 1: { // disp8
        int8_t d = mem_read8((cpu.cs<<4) + get_ip());
        cpu.eip++;
    ...
    case 2: { // disp16
        uint16_t d = mem_read16((cpu.cs<<4) + get_ip());
        cpu.eip += 2;
...
case 0xF7: {   // F7 /x r/m16
    ...
    switch (m.reg) {
        ...
        case 6: { // DIV r/m16
            ...
            if (val == 0 || (dividend / val) > 0xFFFF) {
                Print(L"[CPU] DIV error\n");
                return;
            }

            uint16_t quotient  = (uint16_t)(dividend / val);
            uint16_t remainder = (uint16_t)(dividend % val);
...
case 0x2A: {   // SUB r8, r/m8
    ...
    cpu.eflags = (cpu.eflags & 0xFFFF0000) | (flg & 0x0FD7);
