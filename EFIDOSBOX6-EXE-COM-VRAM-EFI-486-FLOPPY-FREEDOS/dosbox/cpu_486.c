...
void dump_mem(uint32_t phys, uint32_t size)
{
    Print(L"\n=== Dump %05X (%u bytes) ===\n", phys, size);
    ...
    for (uint32_t i = 0; i < size; i++)
    {
        ...
        Print(L"%02X ", mem[phys + i]);

        if (i % 16 == 15)
            Print(L"\n");
    }
}
void dump_code(uint16_t cs, uint16_t ip, int count) {
    uint32_t base = ((uint32_t)cs << 4) + ip;
    Print(L"[CODE] dump CS:IP=%04x:%04x (phys=%05x)\n", cs, ip, base);
    for (int i = 0; i < count; i++) {
        Print(L"%02x ", mem_read8(base + i));
    }
    Print(L"\n");
}
void set_sreg(uint8_t reg, uint16_t val) {
    switch (reg) {
...
void dos_boot_init(void)
{
    ...
    // Segmentos típicos de BIOS al arrancar
    cpu.ds = 0x0000;
    cpu.es = 0x0000;
    cpu.ss = 0x0000;
    cpu.esp = 0xFFFE;
    ...
    //===========================================================================
    // Instalar INT 0 en RAM (NO en F000) - Parada controlada con INT0 de salida
    //
    //mem_write16(0x00000, 0x0500); // offset
    //mem_write16(0x00002, 0x0000); // segment

    // Handler INT 0 en 0000:0500
    //uint16_t base = 0x0500;
    //uint8_t code[] = {
    //    0xFA,       // CLI
    //    0xF4,       // HLT
    //    0xEB, 0xFD  // JMP -3
...
void cpu_step(void)
{
    // Leer SIEMPRE el opcode con fetch8() para avanzar IP correctamente
    uint8_t op = fetch8();
    ...
    case 0xD8: {   // ESC/FPU D8
    uint8_t modrm = fetch8();   // la instrucción es de 2 bytes: D8 + ModRM
    Print(L"[CPU] opcode D8 (FPU ESC) ignorado, modrm=%02x\n", modrm);
    return;
    }
    ...
    case 0xC0: {   // C0 /r r/m8, imm8
    uint8_t modrm = fetch8();
    ModRM m = decode_modrm(modrm);

    uint8_t imm = fetch8();   // shift count
    imm &= 0x1F;              // x86 masks shift count
    ...
    if (m.mod == 3) {
        ...
        ea   = ea16(m);
        addr = segaddr(default_seg16(m), ea);
        val  = mem_read8(addr);
    }
    ...
    case 3: // RCR
        for (int i = 0; i < imm; i++) {
            uint8_t old_cf = (cpu.eflags & FLAG_CF) ? 1 : 0;
            carry = (result & 1);
            result = (result >> 1) | (old_cf ? 0x80 : 0);
            cpu.eflags = (cpu.eflags & ~FLAG_CF) | (carry ? FLAG_CF : 0);
        }
        break;

    case 4: // SHL/SAL
        carry = (result & (1 << (8 - imm))) ? 1 : 0;
        result <<= imm;
        cpu.eflags = (cpu.eflags & ~FLAG_CF) | (carry ? FLAG_CF : 0);
        break;

    case 5: // SHR
        carry = (result >> (imm - 1)) & 1;
        result >>= imm;
        cpu.eflags = (cpu.eflags & ~FLAG_CF) | (carry ? FLAG_CF : 0);
        break;
    ...
    case 7: // SAR
        carry = (result >> (imm - 1)) & 1;
        result = ((int8_t)result) >> imm;
        cpu.eflags = (cpu.eflags & ~FLAG_CF) | (carry ? FLAG_CF : 0);
        break;

    default:
        Print(L"[CPU] C0 /%d no implementado\n", m.reg);
        return;
    }

    // ZF, SF, PF
    set_flag(FLAG_ZF, result == 0);
    set_flag(FLAG_SF, result & 0x80);
    cpu.eflags = (cpu.eflags & ~FLAG_PF) |
                 (((__builtin_popcount(result) & 1) == 0) ? FLAG_PF : 0);
...
case 0xF3: {
    ...
    case 0xA5: {   // REP MOVSW
        while (get_cx() != 0) {
            uint16_t v = mem_read16(segaddr(cpu.ds, get_si()));
            mem_write16(segaddr(cpu.es, get_di()), v);
...
case 0xEB: {   // JMP rel8
    ...
    dump_code(cpu.cs, get_ip(), 16); // DUMP antes de ejecutar el code.
    ...
    if (m.mod == 3) {
        ...
        uint32_t ea = ea16(m);
        uint16_t seg = cpu.segment_override
                       ? cpu.override_seg
                       : default_seg16(m);
        cpu.segment_override = 0;
        val = mem_read16(((uint32_t)seg << 4) + ea);
    }
...
case 0xC7: {   // C7 /0  MOV r/m16, imm16
    ...
    // r/m16 → memoria
    uint32_t ea = ea16(m);   // ea16 consumirá disp8/disp16
    imm = fetch16();         // ahora lees el inmediato
    uint16_t seg = cpu.segment_override ? cpu.override_seg : cpu.ds;
    cpu.segment_override = 0;
    ...
    if (m.mod == 3) {
        ...
        // Log si el destino es CX
        if (m.reg == 1) {
            Print(L"[CX-LOAD] CS:IP=%04X:%04X  CX <= %04X  (from register rm=%d)\n",
                  cpu.cs, get_ip(), val, m.rm);
        }
        ...
        // Segment override o DS
        seg = cpu.segment_override ? cpu.override_seg : cpu.ds;
        cpu.segment_override = 0;
        ...
        // Log si el destino es CX
        if (m.reg == 1) {
            Print(L"[CX-LOAD] CS:IP=%04X:%04X  CX <= %04X  from %04X:%04X (phys=%05X)\n",
                  cpu.cs, get_ip(),
                  val,
                  seg, ea,
                  addr);

            // Dump automático de la zona que alimenta CX
            dump_mem(addr & ~0xFF, 0x100);
        }
    }
    ...
    case 0xF4: { // HLT
        // Para tu emu, lo más sencillo: parar la ejecución DOS 
    ...
    // Log si el destino es AL (reg == 0) o AH (reg == 4)
    if (m.reg == 0 || m.reg == 4) {
        Print(L"[MOV8] CS:IP=%04X:%04X  %s <= %02X  from %04X:%04X (phys=%05X)\n",
              cpu.cs, get_ip(),
              (m.reg == 0) ? L"AL" : L"AH",
              val,
              seg, ea, addr);
...
case 0x74: { int8_t rel = fetch8(); Print(L"[CPU] AFTER: CS:IP=%04x:%04x AX=%04x BX=%04x CX=%04x DX=%04x SI=%04x DI=%04x\n",
      cpu.cs, get_ip(), get_ax(), get_bx(), get_cx(), get_dx(), get_si(), get_di());
if (cpu.eflags & FLAG_ZF) set_ip(get_ip()+rel); return; }
...
case 0xF7: {   // F7 /x r/m16
    ...
    uint8_t modrm = fetch8();
    ModRM m = decode_modrm(modrm);
    Print(L"[CPU] F7 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    ...
    case 6: { // DIV r/m16
        ...
        Print(L"[CPU] F7 DIV: DX=%04x AX=%04x val=%04x dividend=%08x\n",
              get_dx(), get_ax(), val, dividend);

        if (val == 0) {
            // Dump de la zona que está provocando el DIV por cero
            if (m.mod == 3) {
                Print(L"[CPU] F7 DIV by zero on register rm=%d\n", m.rm);
            } else {
                Print(L"[CPU] F7 DIV by zero at addr=%05X (seg=%04X ea=%04X)\n",
                      addr, default_seg16(m), ea);
                dump_mem(addr & ~0xFF, 0x100);   // 256 bytes alrededor
            }
            cpu_interrupt(0);
            return;
        }
    ...
    case 7: { // IDIV r/m16
        ...
        Print(L"[CPU] F7 IDIV: DX=%04x AX=%04x val=%04x dividend=%08x divisor=%04x\n",
              get_dx(), get_ax(), val, dividend, divisor);

        if (divisor == 0) {
            if (m.mod == 3) {
                Print(L"[CPU] F7 IDIV by zero on register rm=%d\n", m.rm);
            } else {
                Print(L"[CPU] F7 IDIV by zero at addr=%05X (seg=%04X ea=%04X)\n",
                      addr, default_seg16(m), ea);
                dump_mem(addr & ~0xFF, 0x100);
            }
            cpu_interrupt(0);
    ...
    case 0x84: {   // TEST r/m8, r8
        uint8_t modrm = fetch8();
        ModRM m = decode_modrm(modrm);
        Print(L"[CPU] AFTER: CS:IP=%04x:%04x AX=%04x BX=%04x CX=%04x DX=%04x SI=%04x DI=%04x\n",
      cpu.cs, get_ip(), get_ax(), get_bx(), get_cx(), get_dx(), get_si(), get_di());
    ...
    case 0xF6: {   // F6 /x r/m8
        uint8_t modrm = fetch8();
        ModRM m = decode_modrm(modrm);
        Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE: AX=%04x BX=%04x CX=%04x DX=%04x\n",
        ...
        if (m.mod == 3) {
            ...
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE caso m.mod==3: AX=%04x BX=%04x CX=%04x DX=%04x\n",
          get_ax(), get_bx(), get_cx(), get_dx());
        } else {
            ea = ea16(m);
            uint16_t seg = default_seg16(m);
        ...
        case 2: { // NOT r/m8
            ...
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE caso 2: AX=%04x BX=%04x CX=%04x DX=%04x\n",
          get_ax(), get_bx(), get_cx(), get_dx());
    ...
    Print(L"[CPU] F6 BEFORE caso 2: AX=%04x BX=%04x CX=%04x DX=%04x\n",
        ...
        case 3: { // NEG r/m8
            ...
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE caso 3: AX=%04x BX=%04x CX=%04x DX=%04x\n",
          get_ax(), get_bx(), get_cx(), get_dx());
            // CF = 1 si val != 0
            if (val != 0) cpu.eflags |= FLAG_CF;
            else cpu.eflags &= ~FLAG_CF;
    ...
    Print(L"[CPU] F6 BEFORE caso 3: AX=%04x BX=%04x CX=%04x DX=%04x\n",
        ...
        case 4: { // MUL r/m8  (AL * r/m8 = AX)
            ...
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE case 4: AX=%04x BX=%04x CX=%04x DX=%04x\n",
          get_ax(), get_bx(), get_cx(), get_dx());
            set_ax(res);
    ...
    Print(L"[CPU] F6 BEFORE case 4: AX=%04x BX=%04x CX=%04x DX=%04x\n",
        ...
        case 5: { // IMUL r/m8  (signed)
            ...
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE case 5: AX=%04x BX=%04x CX=%04x DX=%04x\n",
          get_ax(), get_bx(), get_cx(), get_dx());
            set_ax((uint16_t)res);
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    ...
    Print(L"[CPU] F6 BEFORE case 5: AX=%04x BX=%04x CX=%04x DX=%04x\n",
        ...
        case 6: { // DIV r/m8
            uint16_t ax = get_ax();
            Print(L"[CPU] F6 DIV: AX=%04x val=%04x\n", 
            get_ax(), val);
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE case 6: AX=%04x BX=%04x CX=%04x DX=%04x\n",
          get_ax(), get_bx(), get_cx(), get_dx());
    ...
    Print(L"[CPU] F6 BEFORE case 6: AX=%04x BX=%04x CX=%04x DX=%04x\n",
        ...
        case 7: { // IDIV r/m8
            ...
            Print(L"[CPU] F6 IDIV: AX=%04x val=%04x divisor=%04\n",
          get_ax(), val, divisor);
            ...
            Print(L"[CPU] F6 ModRM : modrm=%02x mod=%d reg=%d rm=%d\n",
          modrm, m.mod, m.reg, m.rm);
    Print(L"[CPU] F6 BEFORE case 7: AX=%04x BX=%04x CX=%04x DX=%04x\n",
          get_ax(), get_bx(), get_cx(), get_dx());
            return;
        }
    ...
    if (m.mod == 3) {
        ...
        ea   = ea16(m);                 // ea16 consume disp8/disp16
        addr = segaddr(cpu.ds, ea);
        dst  = mem_read16(addr);
        imm  = (int8_t)fetch8();        // ahora el imm8
    }
    ...
    case 0x02: { // ADD r8, r/m8
        ...
        set_reg8(m.reg, (uint8_t)res);

        // FLAGS
        set_flag(FLAG_CF, res > 0xFF);
        set_flag(FLAG_ZF, (res & 0xFF) == 0);
        set_flag(FLAG_SF, res & 0x80);
        set_flag(FLAG_OF, ((dst ^ src ^ 0x80) & (dst ^ res) & 0x80) != 0);
...
case 0x91: {
    uint16_t ax = get_ax(), cx = get_cx();
        Print(L"[CPU] 91 AFTER: AX=%04x CX=%04x (eax=%08x ecx=%08x)\n",
          get_ax(), get_cx(), cpu.eax, cpu.ecx);
    set_ax(cx);
    set_cx(ax);
    Print(L"[CPU] 91 AFTER: AX=%04x CX=%04x (eax=%08x ecx=%08x)\n",
          get_ax(), get_cx(), cpu.eax, cpu.ecx);
