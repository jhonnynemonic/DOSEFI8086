...
case 0x02:
{
    ...
    if (lba == 0)
        dst = mem + 0x7C00;          // boot sector
    else
        dst = mem + (es*16 + bx);    // loader-managed buffers
    ...
    return;
       ...
       // ---------------------------------------------------------
// AH = 03h — WRITE SECTORS (CHS)  *** BIOS-COMPATIBLE ***
// ---------------------------------------------------------
case 0x03:
{
    ...
    // ============================================================
    // BIOS REAL MODE MEMORY MAP FOR FLOPPY DISK
    // ============================================================
    uint8_t *src;

    if (lba == 0)
        src = mem + 0x7C00;        // boot sector
    else
        src = mem + (lba * 512);   // sector N → 0000:(N*512)
    ...
    Print(L"[INT13] Wrote LBA=%u from phys=%05X\n",
          lba, (uint32_t)(src - mem));
    ...
    return;
        ...
        case 0x08:
        {
            set_ah(0);
            set_bx(0x0000);
            set_cx((79 << 8) | 18);   // CH=79, CL=18
            set_dx((1 << 8) | 0);     // DH=1, DL=0
            cpu.eflags &= ~FLAG_CF;
            return;
        }
...
{
    cpu_set_ah(0x00);        // AH = 00h (OK)
    cpu_set_al(0x01);        // AL = 01h (extensions present)
...
// ---------------------------------------------------------
// AH = 42h — EXTENDED LBA READ  *** BIOS-COMPATIBLE ***
// ---------------------------------------------------------
case 0x42:
{
    ...
    for (uint16_t i = 0; i < count; i++)
    {
        ...
        if (int13_read_sector_lba(lba + i, dst))
        {
            set_ax(0x0001);
            cpu.eflags |= FLAG_CF;
            return;
    ...
    Print(L"[INT13] LBA42 read: first LBA=%llu → phys=%05X\n",
          lba, (uint32_t)((lba == 0 ? 0x7C00 : lba * 512)));
...
void handle_int(uint8_t intnum)
{
    ...
    // Simular INT 0 real: PUSHF, PUSH CS, PUSH IP y saltar al vector 0
    uint16_t sp = cpu_get_sp();

    // PUSH IP
    sp -= 2;
    mem_write16((cpu.ss << 4) + sp, cpu_get_ip());
    ...
    // PUSH FLAGS
    sp -= 2;
    mem_write16((cpu.ss << 4) + sp, cpu.eflags & 0xFFFF);

    cpu_set_sp(sp);

    // Leer vector 0 (0000:0000)
    uint16_t new_ip = mem_read16(0x00000);
    uint16_t new_cs = mem_read16(0x00002);
