...
extern uint8_t *gExeBufferDiskSTUB;
extern UINTN gExeSizeDiskSTUB;
...
void dosbox_init(void)
{
    ...
    // Reservar 128 MB si aún no existe
    if (!mem) {
        mem = AllocatePool(128 * 1024 * 1024);
        if (!mem) {
    ...
    for (UINTN i = 0; i < 128 * 1024 * 1024; i++)
        mem[i] = 0;

    Print(L"[INIT] Memoria inicializada (128 MB)\n");
...
dos_boot_init();
        ...
        //
    // 0.1. Verificar si hay disquete BIOS_STUB.BIN cargado
    //
    //if (gExeBufferDiskSTUB != NULL && gExeSizeDiskSTUB > 0) {
    //    Print(L"[DOS] Disquete STUB cargado (%lu bytes)\n", (UINT64)gExeSizeDiskSTUB);
    //} else {
    //    Print(L"[DOS] ERROR: No hay disquete STUB cargado\n");
    //    return;
    //}

    //
    // 0.2 Cargar el boot sector (37 bytes) en 0000:0500
    //
    //Print(L"[DOS] Copiando stub a 0000:0500...\n");

    //uint8_t *bootstub = (uint8_t*)gExeBufferDiskSTUB;  // sector 0
    //uint32_t dststub = 0x0500;

    //for (int i = 0; i < 37; i++)
    //    mem_write8(dststub + i, bootstub[i]);

    //Print(L"[DOS] STUB cargado. CS:IP = 0000:0500\n");
    ...
    Print(L"[DISK] img_size=%d\n", (UINT64)gExeSizeDisk);
    detect_geometry((UINT64)gExeSizeDisk);

    Print(L"[INT13 GEOM] is_floppy=%d HEADS=%d SPT=%d CYL=%d\n",
      gDiskGeom.is_floppy,
      gDiskGeom.heads,
      gDiskGeom.sectors_per_track,
      gDiskGeom.cylinders);
    //
    ...
    //
    // 3.1 Inicializar IVT con STUB
    //
    // init_ivt();     -Agenda STUB
    //
