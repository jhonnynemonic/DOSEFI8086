...
// IVT
// ======================================================
// IVT en 0000:0000, cada vector son 4 bytes: IP, CS
void init_ivt(void)
{
    uint32_t ivt_base = 0x00000;

    // INT 13h (0x13) → stub BIOS en 0000:0500
    uint32_t off_13 = 0x13 * 4;
    mem_write16(ivt_base + off_13,     0x0500); // IP
    mem_write16(ivt_base + off_13 + 2, 0x0000); // CS

    // Aquí podrías añadir INT 10h, 19h, etc., si los necesitas.
}
void bios_dispatch(uint8_t intnum)
{
    //
    // 1. Leer registros del invitado desde 0x0100
    //
    uint16_t ax = mem_read16(0x0100);
    uint16_t bx = mem_read16(0x0102);
    uint16_t cx = mem_read16(0x0104);
    uint16_t dx = mem_read16(0x0106);
    uint16_t si = mem_read16(0x0108);
    uint16_t di = mem_read16(0x010A);
    uint16_t es = mem_read16(0x010C);
    uint16_t ds = mem_read16(0x010E);

    //
    // 2. Cargar esos valores en la CPU emulada (usando helpers)
    //
    set_ax(ax);
    set_bx(bx);
    set_cx(cx);
    ...
    //
    // 3. Ejecutar la interrupción real en el host
    //
    handle_int(intnum);

    //
    // 4. Devolver los registros modificados al invitado
    //
    mem_write16(0x0100, get_ax());
    mem_write16(0x0102, get_bx());
    mem_write16(0x0104, get_cx());
    mem_write16(0x0106, get_dx());
    mem_write16(0x0108, get_si());
    mem_write16(0x010A, get_di());
    mem_write16(0x010C, cpu.es);
    mem_write16(0x010E, cpu.ds);

    //
    // 5. Limpiar la señal
    //
    mem_write8(0x00F0, 0x00);
}
...
void dos_boot_init(void)
{
    ...
    //
    // Instalar INT 0 en RAM (NO en F000)
    //
...
case 0xF6: {   // F6 /x r/m8
    ...
    switch (m.reg) {
        ...
        case 6: {
            ...
            // Error: divisor = 0 o overflow
            if (val == 0 || (ax / val) > 0xFF) {
                // INT 0 — Divide Error (simulado)
                handle_int(0x00);
                return;
            }
...
case 0xCF: {   // IRET
    ...
    // Restaurar flags 8086 reales (máscara típica)
...
case 0xCD: {
    ...
    // Leer número de INT (byte después del opcode CD)
    uint8_t intnum = mem_read8((cs << 4) + ip);

    // Avanzar IP (CD xx ocupa 2 bytes)
    ip = (ip + 1) & 0xFFFF;
    set_ip(ip);

    // PUSH FLAGS, CS, IP (como un 8086 real)
    uint16_t sp = cpu_get_sp();
    sp -= 2; mem_write16((cpu.ss << 4) + sp, cpu.eflags & 0xFFFF);
    sp -= 2; mem_write16((cpu.ss << 4) + sp, cs);
    sp -= 2; mem_write16((cpu.ss << 4) + sp, ip);
    cpu_set_sp(sp);
    Print(L"INT opcode CD: intnum=%02X\n", intnum);

    
    // ⭐ INT BIOS 00h–1Fh → dispatcher C
    if (intnum < 0x20) {
        handle_int(intnum);
        return;
    }

    // ⭐ INT DOS 20h–FFh → IVT (FreeDOS)
    uint32_t vec = intnum * 4;
    uint16_t new_ip = mem_read16(vec);
    uint16_t new_cs = mem_read16(vec + 2);
