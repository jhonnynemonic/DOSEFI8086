...
uint8_t  *gExeBuffer = NULL;
UINTN  gExeSize   = 0;
uint8_t *gExeBufferDisk = NULL;
UINTN gExeSizeDisk = 0;
uint8_t *gExeBufferDiskSTUB = NULL;
UINTN gExeSizeDiskSTUB = 0;
...
static EFI_STATUS
LoadFileToMemory(EFI_FILE_PROTOCOL *File, uint8_t **OutBuffer, UINTN *OutSize) {
    EFI_STATUS Status;
    ...
    // 2. Reservar memoria
    uint8_t *Buffer = AllocatePool(FileSize);
    if (!Buffer) {
        return EFI_OUT_OF_RESOURCES;
    }
...
EFI_STATUS EFIAPI efi_fs_disk(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    CHAR16 *FileName = L"freedos.boot.disk.160K.img";   // cámbialo si quieres otro nombre
    ...
    Status = LoadFileToMemory(File, &gExeBufferDisk, &gExeSizeDisk);
    ...
    Print(L"Cargado %s en memoria: %lu bytes\n", FileName, (UINT64)gExeSizeDisk);

    // Aquí ya puedes llamar a dosbox_core_run() o similar desde fuera,
    // usando gExeBufferDisk y gExeSizeDisk como fuente del programa.

    return EFI_SUCCESS;
}

EFI_STATUS EFIAPI efi_fs_stub(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    EFI_STATUS Status;
    ...
    EFI_FILE_PROTOCOL *File;
    CHAR16 *FileName = L"bios_stub.bin";   // cámbialo si quieres otro nombre
    Status = uefi_call_wrapper(Root->Open, 5,
                               Root,
                               &File,
                               FileName,
    ...
    // Cargar el fichero entero en memoria
    Status = LoadFileToMemory(File, &gExeBufferDiskSTUB, &gExeSizeDiskSTUB);
    uefi_call_wrapper(File->Close, 1, File);

    if (EFI_ERROR(Status)) {
        Print(L"Error cargando %s en memoria (Status=%r)\n", FileName, Status);
        gExeBufferDiskSTUB = NULL;
        gExeSizeDiskSTUB   = 0;
        return Status;
    }

    Print(L"Cargado %s en memoria: %lu bytes\n", FileName, (UINT64)gExeSizeDiskSTUB);

    // Aquí ya puedes llamar a dosbox_core_run() o similar desde fuera,
    // usando gExeBufferDiskSTUB y gExeSizeDiskSTUB como fuente del programa.
...
EFI_STATUS efi_load_by_name(CHAR16 *FileName)
{
    ...
    Status = uefi_call_wrapper(BS->HandleProtocol, 3,
                               ...
                               (VOID**)&LoadedImage);
    if (EFI_ERROR(Status)) {
        gfx_printf("[EFI] LoadedImage fallo (Status=%r)\n", Status);
        return Status;
    }

    // 2. Obtener SimpleFS del mismo dispositivo
    EFI_SIMPLE_FILE_SYSTEM_PROTOCOL *SimpleFS;
    Status = uefi_call_wrapper(BS->HandleProtocol, 3,
                               LoadedImage->DeviceHandle,
                               &gEfiSimpleFileSystemProtocolGuid,
                               (VOID**)&SimpleFS);
    if (EFI_ERROR(Status)) {
        gfx_printf("[EFI] SimpleFS no encontrado (Status=%r)\n", Status);
        return Status;
    }

    // 3. Abrir el volumen raíz
    EFI_FILE_PROTOCOL *Root;
    Status = uefi_call_wrapper(SimpleFS->OpenVolume, 2, SimpleFS, &Root);
    if (EFI_ERROR(Status)) {
        gfx_printf("[EFI] OpenVolume fallo (Status=%r)\n", Status);
        return Status;
    }

    // 4. Intentar abrir el archivo directamente
    EFI_FILE_PROTOCOL *File;
    Status = uefi_call_wrapper(Root->Open, 5,
                               Root,
                               &File,
                               ...
                               0);
    if (EFI_ERROR(Status)) {
        gfx_printf("[EFI] No se pudo abrir %s (Status=%r)\n", FileName, Status);
        return Status;
    }

    // 5. Cargar el archivo entero en memoria
    Status = LoadFileToMemory(File, &gExeBuffer, &gExeSize);
    uefi_call_wrapper(File->Close, 1, File);

    if (EFI_ERROR(Status)) {
        gfx_printf("[EFI] Error cargando %s en memoria (Status=%r)\n", FileName, Status);
        gExeBuffer = NULL;
        gExeSize   = 0;
        return Status;
    }

    gfx_printf("[EFI] Cargado %s (%lu bytes)\n", FileName, gExeSize);
    return EFI_SUCCESS;
}
